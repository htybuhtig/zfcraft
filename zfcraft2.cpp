...
map<int,string> blockini_b;
map<int,string> blockini_w;
map<int,int> blockini_fz;
map<int,int> blockini_dd;
int keydown(int c){
...
int readini(){
	...
	ifstream bini("block.ini");
	if(!bini.is_open()){
		cout<<"readini/ error cannot open file block.ini\n";
		return 1;
	}
	string s1,s2;
	int i = 0;
	int i1,i2;
	while(bini>>s1&&s1!="STOP"){
		bini>>s2>>i1>>i2;
		cout<<"readini/blockini i:"<<i<<" s1[b]:"<<s1<<" s2[w]:"<<s2<<" i1[fz]:"
		<<i1<<" i2[dd]:"<<i2<<endl;
		blockini_b[i]=s1;
		blockini_w[i]=s2;
		blockini_fz[i] = i1;
...
string getblockini(int id,bool inworld=0){
	if(id==0) return "";
	if(inworld) return blockini_w[id];
	else return blockini_b[id];
}
...
int loadworld(string s){
	string fn = s+"world.mcw";
	cout<<"loadworld fn:"<<fn<<endl;
...
int loadwpl(string s){
	string fn = s + "wpl.mcb";
	ifstream ifs(fn.c_str(),ios::binary);
	cout<<"loadwpl fn:"<<fn<<endl;
	if(!ifs.is_open()){
		cout<<"open file error!\n";
		return -1;
	}
	...
	for(int i = 0;i<=40;i++){
		...
		bag[i].dd = c2;
		bag[i].zt = c3;
		cnt+=bag[i].id+bag[i].dd+bag[i].zt;
		cnt%=65536;
	}
	ifs.close();
	return cnt;
}
bool loadchk(string s,int a1,int a2){
	cout<<"loadchk/ running\n";
	string fn = s+"chk.txt";
	ifstream ifs(fn.c_str());
	int i1,i2,i3,i4,i5,c1,c2;
	string str;
	ifs>>i1>>i2>>i3>>i4>>i5;
	ifs>>str>>c1>>str>>c2;
	time_t tp = i3;
	...
	play_x = i4;
	play_y = i5;
	cout<<"chkA "<<c1<<" "<<c2<<endl;
	bool f = (a1==c1)&&(a2==c2);
	cout<<"检验结果 "<<(f?"TRUE":"FALSE")<<endl;
	ifs.close();
	return f;
}
int savechk(string s,int a,int b){
	cout<<"savechk/ running\n";
	string fn = "save\\"+s+"\\chk.txt";
	ofstream ofs(fn.c_str());
	ofs<<WORLDSIZE<<" "<<WORLDHEIGHT<<" "<<time(0)<<endl;
	ofs<<play_x<<" "<<play_y<<endl;
	ofs<<"chk1\n"<<a<<endl;//chk 校验和
	ofs<<"chk2\n"<<b<<endl;//chk 校验和
	ofs.close();
	return 0;
...
int loadall(){
	...
	int c1 = loadworld(fn);
	int c2 = loadwpl(fn);
	int r = loadchk(fn,c1,c2);
	printf("loadall chkB:%d %d success:%d\n",c1,c2,r);
	if(r==0){
		cout<<"该世界已被修改，无法保证安全\n";
	}
}
int savewpl(string s){
	cout<<"savewpl/ running\n";
	string fn = "save\\"+s+"\\wpl.mcb";
	ofstream ofs(fn.c_str(),ios::binary);
	//ofs<<WORLDSIZE<<" "<<WORLDHEIGHT<<" "<<time(0)<<endl;
	int cnt = 0;
	for(int i = 0;i<=40;i++){
		ofs<<(char)bag[i].id<<(char)bag[i].dd<<(char)bag[i].zt;
		cnt+=bag[i].id+bag[i].dd+bag[i].zt;
		cnt%=65536;
	}
	ofs.close();
...
int showpos(){
	...
	string s2 = "TPS:"+to_string(tps)+"   ";
	outtextxy(280,0,s2.c_str());
}
int drawwpl(){
	settextstyle(24,0,"Cascadia Code");
	setbkmode(TRANSPARENT);
	setlinecolor(WHITE);
	setfillcolor(BLACK);
	for(int i = 0;i<9;i++){
		...
		if(i==choosewpl){
			setlinecolor(YELLOW);
			fillrectangle(i*64+1,window_y-64+1,i*64+63,window_y-1);
			setlinecolor(WHITE);
		}
		...
		outtextxy(i*64+30,window_y-25,s.c_str());
	}
	setbkmode(OPAQUE);
	settextstyle(24,0,"楷体");
	string s = getblockini(bag[choosewpl].id);
	outtextxy(0,window_y-100,s.c_str());
}
int duidie(int id){
	return blockini_dd[id];//可以堆叠
}
bool canfangzhi(int id){
	return blockini_fz[id];//可以放置
}
bool canfangzhion(int id,int x,int y){//可以放置在
	if(canfangzhi(id)==0) return 0;
	//to do!!
	if(world[x][y].id==0) return 1;
	else return 0;
}
int place_block(int x,int y){
	int wid = bag[choosewpl].id;
	if(canfangzhion(wid,x,y)==1){
		if(bag[choosewpl].dd>0){
			world[x][y].set(wid);
			printf("place_block wid:%d x:%d y:%d\n",wid,x,y);
			bag[choosewpl].dd--;
			bag[choosewpl].checkbug();
		}
	}
}
int mouse_rd(int x,int y){//mouse right down
	int sy = window_y-32*18;
	if(y<window_y-64 && y>sy && x<32*24){
		int dy = (y-sy)/32;
		int dx = x/32;
		int rx = play_x-12+dx;
		int ry = play_y+8-dy;
		float len = sqrt(pow(abs(dx-12),2)+pow(abs(dy-8),2));
		cout<<"mouse_rd dx:"<<dx<<" dy:"<<dy
...
int mouserun(){
	...
	if(msg.uMsg == WM_LBUTTONDOWN){
		...
		if(msg.wheel<0&&choosewpl<8) choosewpl++;
		else if(msg.wheel<0&&choosewpl==8) choosewpl = 0;
		else if(msg.wheel>0&&choosewpl>0) choosewpl--;
		else if(msg.wheel>0&&choosewpl==0) choosewpl = 8;
		else cout<<"WM_MOUSEWHEEL error\n";
		//+-120
	}
}
int drawbag(){
	...
	for(int k = 1;k<=3;k++){
		...
		for(int i = 9*l;i<9+9*l;i++){
			...
			if(bag[i].dd==1) continue;
			string s = to_string(bag[i].dd);
			outtextxy((i-9*l)*64+30,window_y-25-64*l,s.c_str());
		}
	}
	/*
	1 2
...
int boardjtrun(){
	...
	for(int i = 1;i<=9;i++){
		if(keydown('0'+i)){
			cout<<"boardjtrun/ keydown:"<<i<<"\n";
			choosewpl = i-1;
			while(keydown('0'+i)) Sleep(15);
		}
	}
}
int cntmspt(int a){
	if(a==1){
		...
		if(_tps_c<20) _tps_c++;
			...
			_tps_c = 0;
			double t = (GetTickCount() - _tps_t)/20.0;
			_tps_t = GetTickCount();
			tps = 1000.0/t;
		}
